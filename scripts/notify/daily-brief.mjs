/* 팀즈 아침 브리핑 ('26.7) — 매일 08:00 KST에 팀즈 채널로 요약 카드 발송.
   ...
   ③ 신규 등록 일정 (최근 24시간, 휴점 마커 제외)
   + 오늘이 휴점/공휴일이면 상단에 한 줄
...
async function collect() {
  if (MOCK) {
    return {
      ...
      fresh: [{ title: '크리스마스 릴스', channel: '인스타', date: '2026-12-20', end_date: null, owner: '노규빈 선임', kind: null }],
      rmn: buildRmnNotices([
  ...
  /* 신규 등록 — 최근 24h (휴점 마커 제외) */
  const sinceIso = new Date(Date.now() - 24 * 3600e3).toISOString()
  const fresh = (await fetchRows('media_events',
    `select=title,channel,sub,date,end_date,owner,kind&created_at=gte.${encodeURIComponent(sinceIso)}&order=created_at.asc&limit=30`))
    .filter(e => e.kind !== '휴점')
  ...
  return { shoots, fresh, rmn, rmnSkipped }
}

function buildCard({ shoots, fresh, rmn }) {
  const body = []
  ...
  section(`RMN 확인 필요 ${rmn.tentative.length + rmn.tax.length}건`, [
  ...
  section(`신규 등록 일정 ${fresh.length}건`,
    fresh.map(e => `${chLabel(e.channel)} ${e.title} · ${fmtRange(e)}${e.owner ? ` — ${e.owner}` : ''}`))

  const hasContent = shoots.length + fresh.length + rmn.tentative.length + rmn.tax.length > 0
  if (!hasContent) return null
  tb('mediacontent-cal.vercel.app', { isSubtle: true, size: 'Small', spacing: 'Large' })
...
async function main() {
  ...
  const payload = card || buildCard({ ...data, fresh: [{ title: '(빈 브리핑 테스트)', channel: '기타', date: todayISO }] })
  if (DRY) {
