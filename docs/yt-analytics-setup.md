...
로그에 `이 계정에 채널 실적 권한 없음`이 뜨면 그 채널은 **동의한 구글 계정이 관리하지 않는
채널**입니다. 채널 ID는 공개 조회라 권한이 없어도 확인되므로, 채널 ID가 찍혔다고
권한이 있는 것은 아닙니다.

수집 계정 하나가 네 채널을 모두 볼 수 있게 만드는 쪽이 가장 간단합니다.

1. 권한이 빠진 채널의 관리 계정으로 https://studio.youtube.com 접속
2. 좌측 맨 아래 **설정 → 권한**
3. **초대** → 수집에 쓴 구글 계정 이메일 입력
4. 역할은 **뷰어**면 충분합니다 (수익은 안 보여도 되면 뷰어 제한적도 가능)
5. 초대받은 계정의 메일에서 수락
6. Actions → **유튜브 스튜디오 지표** → mode `dry-run` 재실행

토큰을 다시 받을 필요는 없습니다. 권한만 붙으면 같은 토큰으로 조회됩니다.

### 7-1. 관리자인데도 403일 때 (채널마다 토큰 나눠 쓰기)

스튜디오에서 관리자로 보이는데도 403이면, 그 채널은 **브랜드 계정 관리자** 자격으로
관리 중인 것입니다. 유튜브에는 성격이 다른 두 가지 권한이 있고 API는 이를 다르게 봅니다.

| 권한 종류 | 스튜디오 표시 | API에서 보이는 조건 |
|---|---|---|
| 채널 권한 (초대형) | 관리자, 편집자, 뷰어 | 초대받은 계정으로 받은 토큰 |
| 브랜드 계정 관리자 | 똑같이 관리자 | **그 채널로 로그인해 받은 토큰** |

'26.7.31 실측으로 룸넘버는 초대형, 공식과 와지트와 이야호는 브랜드 계정형이었습니다.

채널을 초대형으로 바꿀 수 없으면 **토큰을 여러 개 두면 됩니다.**
수집기는 채널마다 되는 토큰을 찾을 때까지 순서대로 시도합니다.

1. 2번의 동의 절차를 반복하되, **계정 선택 화면에서 그 채널을 고릅니다**
2. 받은 값을 아래 이름으로 등록합니다 (최대 5개)

| 시크릿 | 용도 |
|---|---|
| `YT_OAUTH_REFRESH_TOKEN` | 첫 번째 계정 |
| `YT_OAUTH_REFRESH_TOKEN_2` | 두 번째 계정 |
| `YT_OAUTH_REFRESH_TOKEN_3` | 세 번째 계정 |
| `YT_OAUTH_REFRESH_TOKEN_4` | 네 번째 계정 |

3. dry-run 로그의 `토큰 N이 소유로 보는 채널` 줄로 어느 토큰이 무엇을 잡는지 확인합니다

클라이언트 ID와 보안 비밀은 계정이 달라도 같은 값을 씁니다. 바꿀 필요 없습니다.
