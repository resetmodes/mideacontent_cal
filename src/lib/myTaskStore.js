...
export async function createTodo({ txt, spaceId = null, shared = [] }) {
  const res = await req(api('my_todos'), {
    method: 'POST', headers: REP,
    body: JSON.stringify({ owner_email: myEmail(), txt, space_id: spaceId, shared_with: shared }),
  })
...
const evFrom = r => ({
  ...
  shared: r.shared_with || [], sharedEdit: !!r.shared_edit,
  memo: r.memo, owner: r.owner_email,
})
...
export async function createMyEvent({ title, date, time = null, dur = 60, spaceId = null, shared = [] }) {
  const res = await req(api('my_events'), {
    method: 'POST', headers: REP,
    body: JSON.stringify({
