...
export function parseQuick(input, today = new Date(), opts = {}) {
  ...
  /* 촬영 라벨만 잡힌 경우에도 남은 텍스트의 날짜는 업로드일로 계속 파싱 ('26.7 수정) —
     "7.15 촬영 7.25 현장 스케치"에서 7.25가 제목에 남던 버그 방지 */
  if (!date) {
    ...
    /* 3) 상대 주차: "다음주 목요일", "다다음주 월요일", "지난주 토요일" ('26.7)
       ...
       단일 축약("다음주 금")도 인식 ('26.7 확장) — 단, 요일 글자 뒤에 다른 한글이 붙으면
       제외 ("다음주 월간 보고"의 '월', "다음주 토스"의 '토' 오인 방지). 기간은 ~가 경계 */
    const W = '다다음\\s*주|다음\\s*주|이번\\s*주|지난\\s*주|저번\\s*주|차차주|차주|금주'
    const wr = text.match(new RegExp(`(${W})\\s*([월화수목금토일])(?:요일)?\\s*[~-]\\s*([월화수목금토일])(?:요일)?`))
    const wm = wr ? null : text.match(new RegExp(`(${W})\\s*([월화수목금토일])(?:요일|(?![가-힣]))`))
    /* 4) 상대: 오늘/내일/모레 */
