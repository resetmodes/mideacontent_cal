...
## 4장. 외부(대행사·지점) 공유 — '26.7 거버넌스 확정

- 미러 기본 주소 = **사내 타 팀 전용** (캘린더 노출 — 외부 전달 금지)
- 외부에는 **미러 주소 + `?view=external`** 만 전달: 로그인 없이 새니타이즈된 스펙만
  (담당자명→팀명, 내부 지표 숨김, 캘린더 완전 배제)
- 특정 매체만 열리는 개별 링크: 본 사이트 스펙 탭에서 해당 매체 펼침 →
  **"이 매체 외부 링크 복사"** 버튼 (`?view=external&media=매체명` 형식)
- 미러 도메인을 바꾸면 `src/config.js`의 `MIRROR_URL`도 같이 수정
- **외부인에게는 어떤 계정도 발급하지 말 것** — 뷰어 계정이라도 로그인만 하면
  본 사이트에서 내부 일정을 읽을 수 있음 (RLS 읽기 = 로그인 여부만 검사)
