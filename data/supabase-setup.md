...
### 8-6. 발송·게시 매체 수기 실적 ('26.7.29 — 광고주 공유 리포트용)

카카오톡·푸쉬·인스타그램은 GA4가 집계하지 못해 매체사 리포트 수치를 발급 시 입력받습니다.
입력값을 부킹에 저장해 다음 발급 때 자동으로 채우려면:

```sql
alter table rmn_bookings add column if not exists perf_manual jsonb;
```

- 미실행 시: 공유 리포트에는 정상 표기(스냅샷에 저장됨), **다음 발급 때 프리필만** 안 됨

## 12. RMN 광고주 공유 리포트 ('26.7.29)

캠페인 결과 대시보드를 광고주에게 링크(+임시 비밀번호)로 공유하려면 —
Supabase SQL Editor에서 **`data/rmn-share-setup.sql` 전체를 1회 실행**.

만드는 것:
- `rmn_share` 테이블 — 발급 시점 **스냅샷**(해당 캠페인 데이터만, jsonb) + 토큰 + 만료일(30일)
- RLS: 팀 로그인 계정 조회 / 발급·회수는 `team_writers` / **anon 정책 없음** (직접 조회 불가)
- RPC 2종 (`get_rmn_share_meta`·`get_rmn_share`, security definer) — 광고주는 이 함수로만 열람.
  토큰 일치 + 비밀번호 일치 + 미만료일 때만 스냅샷 반환

확인: 실행 결과 마지막 줄에 `rmn_share ready`가 보이면 완료.

사용: RMN 탭 → 캠페인 행 "광고주 공유" 버튼(GA 데이터 있는 캠페인만) → 발급 →
"링크+비밀번호 복사"로 광고주 전달. 회수 = 같은 모달의 발급 목록에서 삭제.
...
타 팀이 매체 광고, 바이럴을 신청하는 창구입니다. 신청 폼과 진행 현황은
미러 사이트(무로그인)에 전사 공개, 접수 관리는 본 사이트 팀 로그인 전용입니다.

1. Supabase → SQL Editor → `data/lounge-setup.sql` 전체 붙여넣기 → Run (1회)
2. 확인: Table Editor에 `media_requests` 테이블, Storage에 `lounge-src` 버킷이 보이면 완료

미실행 시 라운지 탭이 안내 문구만 표시합니다 (다른 기능 무영향).

권한 구조가 다른 테이블과 다릅니다.

- 신청(INSERT)은 **anon도 허용** — 미러(무로그인)에서 모든 팀이 신청합니다.
  원본 테이블 SELECT는 여전히 로그인 전용
- 전사 공개 진행 현황은 **`lounge_board` 뷰**로만 — 이메일, 문안 상세, 브랜드 소스,
  반려 사유, 팀 메모를 뺀 컬럼만 anon에 열립니다 (뷰 컬럼 추가는 곧 공개, 신중히).
  linked_events(캘린더 일정 id)는 게시 완료 카드의 결과 이미지용으로 포함 —
  일정 자체가 미러 캘린더 anon 정책으로 이미 공개라 추가 공개가 아닙니다
- 상세 열람은 로그인 계정 전체, 배정과 상태 변경은 `team_writers`만
- 이미 14장을 실행한 뒤라면 lounge-setup.sql의 "공개 진행 현황 보드" 블록만
  다시 실행하면 됩니다 (create or replace라 재실행 안전)
- 브랜드 소스 버킷(`lounge-src`)은 **비공개** — 미공개 행사 로고일 수 있어
  event-images(공개)와 달리 로그인 계정만 열람됩니다
- 계획안 등 기밀 문서와 푸시 타겟 고객번호 파일은 시스템에 받지 않습니다 (보유 체크만)

### 14-1. 팀즈 이력 소급 입력 (선택, 오픈 전 권장)

전사 공개 시점에 보드가 비어 있으면 활성 인상이 안 나므로, 팀즈 "바이럴 요청" 채널의
기존 신청 16건을 미리 넣어 둡니다.

1. 위 14장을 먼저 실행합니다
2. SQL Editor → `data/lounge-seed.sql` 전체 붙여넣기 → Run (1회)
3. 확인: 마지막 줄 결과가 `seeded 16`이면 완료

재실행해도 안전합니다 (소급분만 지우고 다시 넣습니다).

시드에서 지어내지 않은 값은 비어 있습니다 — 신청자 이메일(팀즈 메시지에 없음),
담당(배정 이력 미상). **상태는 희망 게시 일정 기준 자동 판정**이라 실제 진행과 다르면
접수 관리함에서 바꾸면 됩니다. 첨부 파일은 넣지 않고 파일명만 메모에 남겼습니다.

## 15. 내 일정 ('26.8 — 개인 투두와 개인 캘린더)

각자 자기 할 일과 개인 일정을 관리하고, 팀 일정과 겹치는지 바로 보는 탭입니다.
1차는 `src/config.js`의 `MYTASK_EMAILS`에 적힌 계정만 탭이 보입니다.

1. Supabase → SQL Editor → `data/mytask-setup.sql` 전체 붙여넣기 → Run (1회)
2. 확인: Table Editor에 `my_spaces`, `my_todos`, `my_events` 세 테이블이 보이면 완료

미실행 시 내 일정 탭이 안내 문구만 표시합니다 (다른 기능 무영향).
재실행해도 안전합니다 (정책을 지우고 다시 만듭니다).

권한 구조가 이 저장소에서 처음 쓰는 방식입니다.

- 기존 테이블은 전부 "로그인하면 다 보임"이지만 여기는 **본인과 공유 대상만** 봅니다
- 읽기는 본인이거나 `shared_with`에 내 이메일이 있을 때, 수정은 본인이거나
  공유 대상이면서 `shared_edit`이 켜져 있을 때, 생성과 삭제는 본인만
- **개인 데이터를 `media_events`에 넣지 않는 것이 이 설계의 핵심**입니다.
  미러 사이트를 열면 `media_events`에 "로그인 없이 누구나 읽기" 정책이 걸리고
  그 정책은 테이블 전체에 적용되므로, 개인 메모를 그 테이블에 넣으면 무로그인으로
  읽히게 됩니다. 그래서 물리적으로 분리된 `my_*` 테이블을 씁니다
- 팀 캘린더 연동을 켠 일정만 `media_events`에 `kind='팀'`으로 한 건 더 생깁니다.
  이때 팀에 공개되는 것은 제목과 날짜뿐이고, 메모와 공유 설정은 넘어가지 않습니다
...
## 16. 회의록 ('26.8 — 사이트에서 바로 녹음하고 요약)

회의록 탭에서 녹음 버튼을 누르면 마이크로 녹음하면서 동시에 받아쓰고,
종료하면 Claude가 결정사항과 할 일로 정리합니다.
`src/config.js`의 `MINUTES_EMAILS`에 적힌 계정만 탭이 보입니다.

1. Supabase → SQL Editor → `data/minutes-setup.sql` 전체 붙여넣기 → Run (1회)
2. 확인: Table Editor에 `meeting_minutes` 테이블, Storage에 `minutes-audio` 버킷
3. 요약을 쓰려면 **Vercel 환경변수 `ANTHROPIC_API_KEY`**가 필요합니다.
   월간 리포트 서술 생성에 이미 넣어 두셨다면 추가 작업이 없습니다

미실행 시 회의록 탭이 안내 문구만 표시합니다 (다른 기능 무영향).
재실행해도 안전합니다.

### 쓰기 전에 알아야 할 브라우저 제약

이건 우리가 고칠 수 있는 게 아니라 브라우저의 한계입니다. 화면에도 같은 내용을 적어 뒀습니다.

- **받아쓰기는 크롬 계열에서만 됩니다.** 사파리와 파이어폭스는 녹음만 되고 전사문이 빕니다.
  그 경우 아이폰 음성 메모의 대본을 "전사문 붙여넣기"로 넣으면 요약은 똑같이 됩니다
- **탭을 닫거나 화면이 잠기면 녹음이 끊깁니다.** 브라우저가 백그라운드 탭의 마이크와
  타이머를 죽이기 때문입니다. 1시간 회의는 화면을 켜둔 노트북에서 하세요
  (설정에서 절전 대기 시간을 늘려 두는 것이 안전합니다)
- **받아쓰기는 브라우저가 음성을 구글 서버로 보내서 처리합니다.** 크롬에 내장된 기능이라
  우리가 우회할 수 없습니다. 기밀 회의는 이 점을 감안해서 판단하세요.
  음성 파일 자체는 우리 비공개 버킷에만 올라갑니다

### 비용

- 전사는 브라우저 내장이라 **0원**입니다 (별도 STT 계약 불필요)
- 요약만 Claude를 씁니다. 1시간 회의 1건에 약 300원, 월 20건이면 약 6천원입니다

### 보관

음성 파일은 비공개 버킷 `minutes-audio`에 들어갑니다. 경로 첫 칸이 소유자 폴더고,
폴더 이름 규칙(이메일의 `@` 와 `.` 을 `_` 로)은 SQL 정책과
`src/lib/minutesStore.js`의 `folderOf`가 글자 그대로 같아야 합니다
(다르면 업로드가 403으로 막힙니다. test-data 18이 이 일치를 감시합니다).

1시간 녹음이 약 15MB이므로 Supabase 무료 1GB로는 약 60시간 분량입니다.
요약이 끝난 회의는 상세에서 삭제하면 음성 파일도 함께 지워집니다.
